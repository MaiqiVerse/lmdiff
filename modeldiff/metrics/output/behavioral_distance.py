...
class BehavioralDistance(BaseMetric):
    ...
    When tokenizers differ, all CE values are BPB-normalized before the formula.
    """
    ...
    def compute(
        ...
        **kwargs: Any,
        ...
        gen_a = engine_a.generate(probes, n_samples=n_samples, max_new_tokens=max_new_tokens)
        gen_b = engine_b.generate(probes, n_samples=n_samples, max_new_tokens=max_new_tokens)
        ...
        for i, probe in enumerate(probes):
            ...
            if use_bpb:
                text_a = probe + outputs_a[i]
                text_b = probe + outputs_b[i]
            ...
            asym_i = (ce_ba - ce_aa) - (ce_ab - ce_bb)
