...
details.runconfig summary { cursor: pointer; font-weight: 600; }
details.runconfig pre {
...
def _build_run_config_block(result: "GeoResult") -> str:
    """The run configuration, inlined in a collapsed ``<details>``.

    HTML embeds a copy where markdown gets only a sidecar (§10.1),
    because HTML's whole purpose is surviving as one file — emailed,
    uploaded, pasted into a wiki — and a sidecar defeats that.

    The embed is **self-describing but not always self-executing**. A
    ``__ref__`` names an out-of-line array payload resolved relative to
    the YAML *file*; an embed is text inside a document and has no such
    anchor. Resolving against the HTML's own directory would be a guess
    that silently succeeds when an unrelated file of the right name sits
    there, which is worse than failing — so the block says so instead of
    pretending, and a loader handed this text refuses on ``__ref__``
    rather than guessing a base path.
    """
    text = getattr(result, "run_config_yaml", None)
    if not text:
        return ""

    from lmdiff._runconfig import REF_KEY

    note = ""
    if REF_KEY in text:
        note = (
            '\n    <p class="subtitle">This configuration is part of a '
            '<strong>bundle</strong> — it references out-of-line array '
            'payloads that are not embedded here. Re-running it requires '
            'the sidecar directory written beside the original '
            '<code>.runconfig.yaml</code>; the copy below is for reading.'
            '</p>'
        )

    return (
        '\n  <details class="runconfig">'
        '\n    <summary>Run configuration (click to expand)</summary>'
        f'{note}'
...
def render(
    ...
    **_unused,
    ...
    runconfig_html = _build_run_config_block(result)
