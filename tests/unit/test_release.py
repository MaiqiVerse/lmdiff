"""Release-metadata smoke tests for the v0.3.2 release (commit 1.12)."""
...
def test_lmdiff_dunder_version_is_0_3_2():
    assert lmdiff.__version__ == "0.3.2"


def test_pyproject_version_is_0_3_2():
    text = (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert re.search(r'^version\s*=\s*"0\.3\.2"\s*$', text, re.MULTILINE), text


def test_changelog_has_v0_3_2_section():
    text = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert re.search(r"^## \[0\.3\.2\]", text, re.MULTILINE), (
        "CHANGELOG.md missing [0.3.2] heading"
    )
