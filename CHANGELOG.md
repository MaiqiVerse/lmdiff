...
- **Domains below a minimum measurable fraction no longer report a share** (``min_valid_fraction``, default ``0.5``). Dropping invalid probes is necessary but not sufficient: a domain can clear the per-probe filter with so few probes left that its aggregate measures the surviving probes rather than the domain. On the Llama-2-7B calibration, 9 of 100 ``longbench_2wikimqa`` probes fit the 4096-token base window — and those 9 are the short left tail, the probes that test long-context capability least. They yielded long-context shares of 27.6 % (``temp_1.5``) and 18.4 % (``chat``) plotted beside domains measured on all 100 probes. ``compute_domain_status`` now reclassifies a below-floor domain as ``variant_only`` (when variant-only coverage clears the floor) or ``out_of_range`` (otherwise), so both ``share`` and ``pdn`` become ``None``. Effect on the calibration: long-context moves off ``partial`` for all seven variants — ``variant_only`` for ``yarn`` / ``long`` / ``code``, ``out_of_range`` for ``math`` / ``chat`` / ``temp_1.5`` / ``system_prompt`` — and each variant's remaining four domains renormalize to 1.0. All non-long-context domains are ``full`` and unaffected. ``min_valid_fraction=0.0`` restores pre-floor behaviour exactly. Rationale and the "why 0.5" argument: ``docs/methodology/normalization.md`` §"Minimum valid fraction".
...
- **``lmdiff._validity.DEFAULT_MIN_VALID_FRACTION``** (= ``0.5``) and the ``min_valid_fraction`` parameter on ``compute_domain_status`` and ``run_family_pipeline``. Documented and overridable rather than a magic constant; validated to ``[0.0, 1.0]`` with a ``ValueError`` outside that range.
...
- **Drift-magnitude colour bins converted to Formula A units.** ``_DRIFT_BIN_EDGES`` moves from ``0.025 / 0.05 / 0.10 / 0.20`` to ``0.25 / 0.5 / 1.0 / 2.0``. Formula A exceeds Formula B by ``√T̄_d`` (5.7–11 on short-prompt domains), so the identical ×2 ladder shifted one decade is a unit conversion, not a retune — same structure, spacing, and labels. Unconverted, 26 of 28 measured cells landed in the top bin and the left pane carried no information. Legend text is now derived from the edge constants rather than restated, so the two cannot drift apart again.
- **``UndifferentiatedFinding``** — new finding type for a variant whose top domain does not clearly lead. See the ``SpecializationPeakFinding`` margin change under Fixed.
...
- **Findings no longer name a domain the validity framework excluded.** ``MostLikeBaseFinding`` / ``BiggestMoveFinding`` source per-domain drift from ``GeoResult.domain_heatmap()``, which is deliberately validity-unaware (raw ``‖δ‖`` over surviving probes). Without a filter the showcase figure reported "Most like base: math on long-context" in its sidebar while rendering "—" for that exact cell in both panes — sourced from the 9 probes that happened to fit the base context window. Findings now drop ``variant_only`` / ``out_of_range`` cells; ``domain_heatmap()`` itself is unchanged for its other callers, and results without ``domain_status`` (v0.2.x, v1–v5 loads) are unaffected.
- **``SpecializationPeakFinding`` now requires a margin over the runner-up**, not just a 30 % share (``_SPECIALIZATION_PEAK_MARGIN`` = 5 pp). The old rule asked whether the peak was large, never whether it was ahead. Dropping long-context renormalized each row over four domains instead of five, which lifted ``chat``'s peak from 29.9 % (silent) to 32.0 % (a specialization claim) on a 2.5 pp lead — the floor fix manufactured a finding for one of the two variants with the weakest claim to one. 5 pp is 2× the 2 pp sample-decode tolerance the calibration regression already grants this metric, rounded up. Below the margin the variant now emits ``UndifferentiatedFinding`` rather than falling silent, since silence is indistinguishable from "not measured": ``chat: no dominant domain (math 32%, code 29%, commonsense 23%)``. On the v0.4.1 calibration this suppresses ``chat`` and ``temp_1.5`` and leaves the other five (margins 15.2–67.7 pp) unchanged.
...
### Fixed
- **``lmdiff.family()`` now loads variant engines lazily** (Fix 4 in PR #15). The v0.4.0 cutover regressed v0.3.2's lazy-load behavior (L-029): `_api.family()` was eagerly building every unique-model variant engine *before* `run_family_pipeline` started, so the 7-variant Llama family had 6 engines (~14 GB each in BF16) sitting in memory simultaneously. The pipeline's existing look-ahead-by-one release machinery was a no-op because the caller's `engines: list` kept every weight alive until `family()` returned. HF's `device_map="auto"` silently spilled the overflow to CPU, completing the run but tanking throughput. Fix: `run_family_pipeline` now accepts an `engine_factory: Callable[[Config], (Engine, bool)]` alongside the existing `variant_engines` dict (mutually exclusive). `_api.family()` and `_api.compare()` build the base engine eagerly, then pass a validating factory; the pipeline constructs each variant on cache-miss inside the loop and `.close()`-s it via look-ahead-by-one release. Peak resident is back to 2 (base + active variant). See L-032. Backward-compatible: the eager `variant_engines` mode is preserved for the 17 unit-test callsites that inject Mock engines.
...
- Tests: 450 unit tests pass (was 438 pre-Fix-3, 444 post-Fix-3; +6 ``test_lazy_engine_loading.py`` cases asserting peak-resident engines ≤ 2 in factory mode, anchor reuse collapses to one factory call, runtime-only variants don't trigger the factory, factory exception cleans up partial engines). The calibration regression tests run only on GPU and remain the hard contract for cutover safety.
